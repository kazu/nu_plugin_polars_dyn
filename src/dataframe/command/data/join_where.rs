...
impl PluginCommand for LazyJoinWhere {
    ...
    fn examples(&self) -> Vec<Example<'_>> {
        vec![Example {
            ...
            example: r#"let df_a = ([[name cash];[Alice 5] [Bob 10]] | polars_dyn into-lazy)
    let df_b = ([[item price];[A 3] [B 7] [C 12]] | polars_dyn into-lazy)
    $df_a | polars_dyn join-where $df_b ((polars_dyn col cash) > (polars_dyn col price)) | polars_dyn collect"#,
            result: Some(
