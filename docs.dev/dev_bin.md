...
- `dev/src/logfmt.rs` — `ScanSource` の impl。`name = "logfmt"`、接尾辞 `.logfmt` / `.logfmt.seek.zst`。
  `--opts` は `polars_logfmt::LogfmtScanOpts` の JSON(`line_filter` は部分文字列、`schema`、
  `batch_size`、`n_threads`、`aligned_cols_cnt`、ssh の `cmd`)。
- `dev/tests/open_logfmt.rs` — `.logfmt` / `.logfmt.seek.zst` の統合テスト。
...
polars_dyn open app.logfmt.seek.zst | polars_dyn collect
...
`.logfmt.seek.zst` は seekzstdsep 形式の seekable zstd。`polars_dyn collect --streaming` は
